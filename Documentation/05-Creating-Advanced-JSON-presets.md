>[!TIP]
> Open the presets to see how they were created.

# Creating Advanced JSON v3 
...
| Body plus separate hair, head, cloth or outline mesh | `AuxiliaryMeshPath` |
...
## 6.1 Costume retexture: keep the mesh and replace its materials
...
## 6.2 Weapon retexture: keep every weapon mesh
...
## 6.3 Weapon replacement: single mesh and multi-part weapons
...
## 6.4 Split costume: body plus an auxiliary mesh
...
## 6.5 Hide a native outline and install a dedicated auxiliary outline

Some characters keep their outline in a separate Skeletal Mesh Component. Replacing only `BodyPath` leaves the old silhouette visible. The stable correction is:

1. Match and hide the exact native outline component.
2. Load the replacement body normally.
3. Load a dedicated outline mesh as `AuxiliaryMeshPath`.
4. Apply outline Material Instances in the exact auxiliary slot order.
...
```json
{
  ...
  "AuxiliaryMeshPath": "/Game/mods/ExampleAuthor/OutlineCostume/Mesh/SK_OutlineCostume_Outline.SK_OutlineCostume_Outline",
  "AuxiliaryClearMaterialOverrides": true,
  "AuxiliaryMaterials": [
    {
      "SlotIndex": 0,
...
## 6.8 Face materials and Morph Target variants

The complete example in section 6.1 already demonstrates all supported face/body customization fields:
...
## 6.9 Correction-field reference
...
| `AuxiliaryMeshPath` | Full Skeletal Mesh object path | Loads one separate hair/head/cloth/outline mesh on the available auxiliary follower component. |
...
## 6.10 Stability rules learned from corrected presets
